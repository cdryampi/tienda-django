<template>
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
      ...
      <td class="px-6 py-4">
        <button @click="removeItem" class="bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded shadow-md">
          ❌ Eliminar
        </button>
      </td>
    </tr>
  ...
  const subtotal = computed(() => {
    return (parseFloat(props.product.price.replace("€", "").replace(",", ".")) * quantity.value).toFixed(2) + " €";
  });
  
  // ✅ Emitir evento `@updated` correctamente
  const updateQuantity = (newQuantity) => {
    quantity.value = newQuantity;
    emit("updated", { product_id: props.product.product_id, quantity: newQuantity });
  };
